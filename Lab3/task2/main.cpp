...
void mouseHandler(int event, int x, int y, int flags, void* userdata)
{
    ...
    if (event == cv::EVENT_LBUTTONDOWN)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            avg_BGR[i] = channel_mean(img, y, x, i, ksize);
        }
        cout << "Pixel (" + to_string(x) + ", " + to_string(y) + "), BGR     = (" + to_string(BGR[0]) + ", " + to_string(BGR[1]) + ", " + to_string(BGR[2]) + ")" << endl;
        cout << "Pixel (" + to_string(x) + ", " + to_string(y) + "), avg_BGR = (" + to_string(avg_BGR[0]) + ", " + to_string(avg_BGR[1]) + ", " + to_string(avg_BGR[2]) + ")" << endl;
    }
}
